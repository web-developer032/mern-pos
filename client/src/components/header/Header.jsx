...
const Header = ({ setSearched = () => {} }) => {
  ...
  const logout = () => {
    if (window.confirm("Are you sure you want to log out?")) {
      ...
      navigate("/login");
      message.success("The logout process is successful.");
    }
  };
    ...
    <div className="border-b mb-6">
      <header className="py-4 px-6 flex justify-between items-center gap-10">
        ...
        <div className="menu-links">
          <Link to="/" className={`menu-link ${pathname === "/" && " text-[#40a9ff]"} `}>
            <HomeOutlined className="md:text-2xl text-xl" />
            <span className="md:text-xs text-[10px]">Home page</span>
          </Link>
          <Badge count={basketNumber} offset={[0, 0]} className="md:flex hidden">
            <Link to="/cart" className={`menu-link ${pathname === "/cart" && " text-[#40a9ff]"} `}>
